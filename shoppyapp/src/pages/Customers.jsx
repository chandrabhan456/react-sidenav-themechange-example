...
function Orders() {
  ...
  const [sort, setSort] = useState({ keyToSort: "CustomerID", direction: "asc" });
 ...
 function handleedit(e){
  ...
  const ordersData3 = customersData;
  ...
  const records1 = ordersData3.filter((el) => {
    if (el === "") {
      ...
      return (
        ...
        el.CustomerEmail.toLowerCase().includes(InputText) ||
        el.Location.toLowerCase().includes(InputText) ||
        el.Week.toLowerCase().includes(InputText) ||
        el.Budget.toLowerCase().includes(InputText) ||
        el.ProjectName.toLowerCase().includes(InputText)
      );
    }
  });
    ...
    <div className="mt-0 h-screen bg-[#B6BBC4] dark:bg-[#1d2041]" >
      ...
      >
        <TableHeader >
          ...
          >
            {customerHeader.map((column) => (
              <TableHeaderCell key={column.Key} id='headercellc' style={{ paddingLeft: "60px" }}>
                {column.label}
                { <BiSortAlt2
                  id="UpDown-icon"
          ...
          {handleDataSort(records)
            ...
            .map((item) => (
              ...
              >
                ...
                >
                  <TableCellLayout style={{ paddingLeft: "60px" }} media={<img   style={{ width: "50px", height: "50px", borderRadius: '50%' }} src={item.CustomerImage} alt="Avatar" />}>
                    {item.CustomerName}
                  </TableCellLayout>
                </TableCell>
                ...
                >
                  <TableCellLayout style={{ paddingLeft: "60px" }}>
                    <span style={{borderRadius:'3px',padding:'0.3rem',color:'white',display:"inline-block",width:"100px",backgroundColor: item.StatusBg}} className="items-center">
                    {item.Status}
                  </span>
                  </TableCellLayout>
                </TableCell>
